         ...
         >
            ...
            {hasAudio ? (
                ...
                <div className="text-center p-8 relative z-10">
                    ...
                    <p className="text-brand-300/50 text-[10px] mt-2 tracking-wide font-bold">OR SKIP / PASTE STREAM LINK</p>
                </div>
            )}
            <input
                type="file"
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 w-full">
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl p-3">
                <input
                  ...
                  placeholder="Paste streaming link (https://...)"
                  value={streamLink}
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    triggerImpulse('click', 0.6);
                    onSetAudioLink(streamLink);
                    setStreamLink('');
                  }}
                  className="px-3 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg text-xs font-bold tracking-widest disabled:opacity-50"
                  disabled={!streamLink.trim()}
                >
                  LINK
                </button>
              </div>
              <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-3 text-xs text-gray-300">
                <div className="flex flex-col">
                  <span className="font-bold text-white/80">No track yet?</span>
                  <span className="text-[10px] text-gray-400">You can add audio later in Step 4.</span>
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); onClearAudio(); setStreamLink(''); }}
                  className="px-3 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg font-bold tracking-widest"
                >
                  ADD LATER
                </button>
              </div>
